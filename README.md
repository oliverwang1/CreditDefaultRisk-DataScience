...
In this case, if a homeowner defaults, the bank loses a million bucks.  This is obviously horrible for the bank so we can swap what's positive and what's negative in the confusion matrix:  
![Confusion Matrix](https://github.com/osuhomebase/CreditDefaultRisk-DataScience/blob/master/Assets%20For%20Presentation/Images/SwapConfusionMatrix.png)  
The bank would probably predict that all mortgages will fail and not loan anyone any money.  This would be horrible for everyone else, and I can tell you from experience, it kind of did suck immediately following the financial crash of 2008 when banks overcompensated for doing the opposite during the previous boom.  The Confusion Matrix would look like the one below:  
